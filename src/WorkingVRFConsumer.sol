...
contract WorkingVRFConsumer is VRFV2PlusWrapperConsumerBase, ConfirmedOwner {
    ...
    function calculateRequestPrice(uint32 _callbackGasLimit, uint32 _numWords) external view returns (uint256) {
        return i_vrfV2PlusWrapper.calculateRequestPrice(_callbackGasLimit, _numWords);
    }

    function calculateRequestPriceNative(uint32 _callbackGasLimit, uint32 _numWords) external view returns (uint256) {
        return i_vrfV2PlusWrapper.calculateRequestPriceNative(_callbackGasLimit, _numWords);
    }
